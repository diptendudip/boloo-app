#!/usr/bin/env python3
"""
Load taxonomy seed data into database
"""
import sys
import os
import json
...
def load_taxonomies():
    ...
    try:
        ...
        existing_count = db.query(Taxonomy).count()
        if existing_count > 0:
            print(f"✓ Taxonomies already loaded ({existing_count} entries)")
            return
        ...
        print(f"Loading {len(taxonomies_data)} taxonomies...")

        for type_val, key, label_en, label_hi, metadata in taxonomies_data:
            taxonomy = Taxonomy(
                type=type_val,
                key=key,
                label_en=label_en,
        ...
        db.commit()
        print(f"✓ Successfully loaded {len(taxonomies_data)} taxonomies")
